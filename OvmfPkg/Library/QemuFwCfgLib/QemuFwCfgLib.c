  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "[Sunce] QemuFwCfgGetDataFromCache Loaded\n"));
  ...
  DEBUG ((DEBUG_INFO, "[Sunce] Item:0x%x Size:0x%x\n", Item, *Size));
  ...
  INT32 ReservedSize = HobSize;

  Ptr = FwCfgData;
  while (ReservedSize > 0 ){
    DEBUG ((DEBUG_INFO, "[Sunce] while Loaded: HobReservedSizeSize is %d\n", ReservedSize));
    FwCfgInfo = (FW_CFG_INFO *)Ptr;
    DEBUG ((DEBUG_INFO, "[Sunce] while Loaded: FwCfgInfo.FwCfgItem is 0x%x\n", FwCfgInfo->FwCfgItem));
    DEBUG ((DEBUG_INFO, "[Sunce] while Loaded: FwCfgInfo.DataSize is 0x%x\n", FwCfgInfo->DataSize));
