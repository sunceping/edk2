    ...
    )
{
    ...
    SumDataSize = 0;
    for (Index = 0; Index < CACHE_FW_CFG_COUNT; Index++){
    ...
    if (SumDataSize < (sizeof (EFI_E820_ENTRY64) + CACHE_FW_CFG_COUNT * sizeof(FW_CFG_INFO))){
        return EFI_ABORTED;
    }
    ...
    DEBUG((DEBUG_INFO, "[Sunce] BuildGuidHob Pass\n"));
