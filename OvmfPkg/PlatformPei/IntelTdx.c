 ...
 */
EFI_STATUS
TdxPeiBuildTdxMeasurementGuidHob (
  UINT32  RtmrIndex,
  UINT32  EventType,
  UINT8   *EventData,
  UINT32  EventSize,
  ...
  )
{
  ...
  Status = TdxPeiHashAndExtendToRtmr (
                                RtmrIndex,
                                HashData,
                                (UINTN)HashDataLen,
                                Digest,
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: TdxPeiHashAndExtendToRtmr failed with %r\n", __func__, Status));
    return Status;
  }

  Status = TdxPeiBuildTdxMeasurementGuidHob (
                                       RtmrIndex,
                                       EventType,
                                       EventData,
                                       EventSize,
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: TdxPeiBuildTdxMeasurementGuidHob failed with %r\n", __func__, Status));
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "%a:: Install CC Measurement PPI is %r.\n", __func__, Status));
